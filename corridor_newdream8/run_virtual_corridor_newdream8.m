function run_virtual_corridor_newdream8()
%%RUN_VIRTUAL_CORRIDOR_NEWDREAM8()
...
% file with corridor
corridor_location       = 'saved\virtual_corridor_newdream8_960x540_20210705.mat';
...
% corridor mask
if flip_on(1)
    corridor_texture = Screen('MakeTexture', window, corridor_mask(:, end:-1:1, :));
else
...
if n_independent_screens == 2
    if flip_on(2)
        corridor_texture2 = Screen('MakeTexture', window2, corridor_mask(:, end:-1:1, :));
    else
...
for i = 1 : size(dot_mask, 3)
    ...
    if flip_on(1)
        dot_texture(i) = Screen('MakeTexture', window, dot_mask(:, end:-1:1, i));
    else
    ...
    if n_independent_screens == 2
        if flip_on(2)
            dot_texture2(i) = Screen('MakeTexture', window2, dot_mask(:, end:-1:1, i));
        else
